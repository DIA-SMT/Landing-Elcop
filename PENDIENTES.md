...
| 31 | ⚠️ **El prototipo de Replit guarda las contraseñas en texto plano — decidido: se despublica** | Félix Agustín Paz / Dirección de IA | En `artifacts/api-server/src/routes/auth.ts`, `checkPassword` compara con `===` aunque la columna se llame `passwordHash`, y `replit.md` trae credenciales de prueba versionadas. **Decisión del 12/8/2026: despublicar, no arreglar** — el portal real ya está en producción y el prototipo no cumple función. Mensaje enviado a Félix pidiendo: (1) despublicarlo, (2) confirmar si alguna vez hubo datos reales (si sí, exportar y borrar la base del Repl), (3) la URL para verificar desde afuera que quedó abajo. El riesgo mayor no es el prototipo: es una persona real registrada con una contraseña que reusa en otros lados, legible en texto plano. |
...
| 28 | ~~Calendario de encuentros de la cohorte~~ | ELCOP | **Resuelto el 11/8/2026.** Llegó el documento oficial "Calendarización para la web" y las 20 clases de mayo a julio están cargadas en `lib/portal/calendario.ts`: Mis clases y el panel muestran datos reales. Quedan tres derivados: el **registro de asistencia** (sin él, el panel dice "sin registro" en vez de un porcentaje falso), los **horarios y aulas** (el documento no los trae; se muestran sólo fechas) y los **materiales** de cada clase. |
...
| 24 | **Reemplazo del video del telón institucional** | ELCOP / Dirección de IA | El video `hero-unsta.mp4` **se sacó de la rotación el 12/8/2026**: el generador había deformado el sello de la UNSTA (letras inventadas donde va el lema) y al bajar el velo del hero quedó a la vista — se verificó con zoom sobre el fotograma. Quedan dos videos rotando. Si se quiere un tercero: regenerar la animación desde otra foto del telón, o filmarlo; antes de volver a listarlo, revisar el sello con zoom. Los archivos siguen en `public/video/`. |
