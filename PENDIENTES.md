...
| ~~Que `estadisticas.smt.gob.ar:5000` mande la cadena completa de certificados~~ | **Resuelto el 10/8/2026.** Lo arregló infraestructura en el servidor, que era el camino bueno: ahora manda el intermedio de Sectigo y el raíz, y Node valida sin ayuda. Verificado seis veces seguidas con `rejectUnauthorized: true` y sin CA extra. `CIDITUC_TLS_INSEGURO` dejó de tener uso; sacarla de los `.env.local` donde haya quedado. |
